...
const ChatWithHirer = () => {
    ...
    const getChatId = () => {
        ...
        const ids = [currentUserId, hirerId].sort();
        return ids.join('_');
    };
    ...
    const fetchHirerInfo = async () => {
        try {
            const response = await api.get(`/users/${hirerId}`);
            if (response.data.success) {
                setHirer(response.data.data);
            }
        } catch (error) {
            console.error('Error fetching hirer info:', error);
            // Don't block chat - just use default placeholder
            setHirer({ fullName: 'Hirer', _id: hirerId });
        } finally {
            setLoading(false);
        }
    };
    ...
    const loadChatHistory = async () => {
        try {
            ...
            const response = await api.get(`/messages/chat/${currentUserId}/${hirerId}`);
            if (response.data.success) {
                        ...
                        <div className="bg-gradient-to-r from-[#0B4B54] to-[#0D5A65] p-6 text-white">
                            <div className="flex items-center gap-4">
                                <div className="w-14 h-14 rounded-full bg-white/20 flex items-center justify-center text-white font-bold text-xl backdrop-blur-sm">
                                    {hirer?.fullName?.charAt(0).toUpperCase() || 'H'}
                                </div>
                                <div>
                                    <h2 className="text-2xl font-bold">{hirer?.fullName || 'Hirer'}</h2>
                            ...
                            {messages.length === 0 ? (
                                <div className="flex flex-col items-center justify-center h-full text-center">
                                    ...
                                    <p className="text-gray-600 max-w-md">
                                        Send a message to {hirer?.fullName} to discuss the job details,
                                        confirm your availability, or ask any questions.
                                    </p>
                    ...
                    <div className="mt-6 bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <button className="px-4 py-3 bg-[#F4FBFA] text-[#0B4B54] rounded-lg hover:bg-[#D3E4E7] transition-colors text-left">
                                <p className="font-semibold">Confirm Availability</p>
                                <p className="text-xs text-gray-600 mt-1">Let them know you're ready</p>
                            </button>
                            <button className="px-4 py-3 bg-[#F4FBFA] text-[#0B4B54] rounded-lg hover:bg-[#D3E4E7] transition-colors text-left">
                                <p className="font-semibold">Ask Questions</p>
                                <p className="text-xs text-gray-600 mt-1">Clarify job details</p>
                            </button>
                            <button className="px-4 py-3 bg-[#F4FBFA] text-[#0B4B54] rounded-lg hover:bg-[#D3E4E7] transition-colors text-left">
                                <p className="font-semibold">View Job Details</p>
                                <p className="text-xs text-gray-600 mt-1">Check shift information</p>
                            </button>
                        </div>
                    </div>
                </div>
